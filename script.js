...
// Handle user query
sendQueryBtn.addEventListener("click", () => {
  const query = healthQueryInput.value.trim();
  if (!query) return;
  ...
  // Fake AI Doctor reply (you can replace this with real API later)
  setTimeout(() => {
    appendMessage("bot", "🤖 I'm your AI Doctor! Based on what you said: \"" + query + "\", I recommend you consult a real doctor for detailed advice.");
  }, 600);
});

// Allow "Enter" key to send
healthQueryInput.addEventListener("keypress", (e) => {
  if (e.key === "Enter") {
    sendQueryBtn.click();
  }
});
